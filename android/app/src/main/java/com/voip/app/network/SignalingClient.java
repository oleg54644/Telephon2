...
import com.voip.app.ui.ActiveCallActivity;
...
public class VoipService extends Service implements SignalingClient.Listener {
    ...
    private static final long WATCHDOG_INTERVAL = 15_000L;
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        if (signalingClient == null || !signalingClient.isOpen()) connectToServer();
        return START_STICKY;
    }

    private void connectToServer() {
        try {
            if (signalingClient != null) { signalingClient.disconnectGracefully(); signalingClient = null; }
            signalingClient = new SignalingClient(SERVER_URL, UDP_PORT, this);
            if (myNumber != null) signalingClient.setMyNumber(myNumber);
            SharedPreferences prefs = getSharedPreferences("voip_prefs", MODE_PRIVATE);
    ...
    public void onDisconnected() {
        sendBroadcast(new Intent(BROADCAST_DISCONNECTED));
    ...
    public void onRegistered(String number) {
        myNumber = number;
        Intent b = new Intent(BROADCAST_REGISTERED);
    ...
    public void onIncomingCall(String callId, String from, String callerName, String callerIp, int callerUdpPort) {
        ...
        String displayName = (callerName != null && !callerName.isEmpty())
            ? callerName + " (" + from + ")" : from;
    ...
    @Override public void onError(String message) { Log.e(TAG, "Signaling error: " + message); }

    // ─── Публичные методы ────────────────────────────────────────────────────

    public void makeCall(String toNumber) {
        if (signalingClient != null && signalingClient.isOpen()) signalingClient.call(toNumber);
    }

    public void answerCall(String callId) {
        if (signalingClient != null) signalingClient.answerCall(callId);
        // Запускаем аудио сразу при ответе
        if (callId != null) audioEngine.startCall("45.128.204.171", UDP_PORT, callIdToInt(callId));
    }
    ...
    public void hangup() {
        if (currentCallId != null && signalingClient != null) signalingClient.endCall(currentCallId);
        audioEngine.stopCall();
        cancelCallNotification();
        currentCallId = null;
    }
    ...
    private void showIncomingCallNotification(String callId, String from) {
        ...
        getSystemService(NotificationManager.class).notify(NOTIF_ID_CALL, n);
    }

    private void cancelCallNotification() {
        getSystemService(NotificationManager.class).cancel(NOTIF_ID_CALL);
    }
    ...
    private void registerNetworkCallback() {
        ...
        } catch (Exception e) { Log.e(TAG, "NetworkCallback error: " + e.getMessage()); }
    }
